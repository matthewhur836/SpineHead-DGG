...
grammarReplaceAll::"usage"="grammarReplaceAll..."
...
AppendTo[arrGrammarPDF,ruleRateFunc/.grammarPDF[dist_,var_]-> (var-> Hold[RandomVariate[dist]])]
,
If[Head[ruleRateFunc] === Times,
Map[Function[{term},
If[Head[term] === grammarPDF,AppendTo[arrGrammarPDF,term/.grammarPDF[dist_,var_]-> (var-> Hold[RandomVariate[dist]])];, AppendTo[arrInputParamsExp,term] ];],ruleRateFunc];
];
];
...
ndsolution = NDSolve[{
\!\(\*SuperscriptBox["curProbFunc", "\[Prime]",
MultilineFunction->None]\)[gGrammarTimeVar]==-totalRateFunc*curProbFunc[gGrammarTimeVar],curProbFunc[0]==1},curProbFunc,{gGrammarTimeVar,0,timeDerivLimit},Method->{ EventLocator,"Event"-> (curProbFunc[gGrammarTimeVar]-randomNumberTime)}(*,PrecisionGoal-> 4 DO NOT USE THIS- see remark above*)];
...
If[debug,Print["timeEventOccured = ",timeEventOccured];];
If[debug,Print["timeDerivLimit = ",timeDerivLimit];];
If[debug,Print["bFoundTimeOfEvent = ",bFoundTimeOfEvent];];

If[debugTiming,Print["inside findNextRuleExec timingNDsolve 2 = ",timingNDsolve];];
...
(*test the result*)
If[Chop[totRateProbFunc[timeEventOccured]-randomNumberTime,defPrecisionNDSolve] !=  0 && totRateProbFunc[timeEventOccured] < randomNumberTime,
Print["ERROR: Event locator missed the event!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"];
Print["ERROR: totRateProbFunc[timeEventOccured] < randomNumberTime"];
Print["randomNumberTime = ",randomNumberTime];
Print["timeDerivLimit = ",timeDerivLimit];
Print["totalRateFunc/.gGrammarTimeVar\[Rule] 0 = ",totalRateFunc/.gGrammarTimeVar-> 0];
Print["timeEventOccured = ",timeEventOccured];
Print["totRateProbFunc[timeEventOccured] = ",totRateProbFunc[timeEventOccured]];
Print["totRateProbFunc[timeEventOccured/2] = ",totRateProbFunc[timeEventOccured/2]];
gValidSimulation = False;
];
